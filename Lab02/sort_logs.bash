...
#---------------------------------------
# $Author: ee364d27 $
# $Date: 2017-09-03 21:40:17 -0400 (Sun, 03 Sep 2017) $
#---------------------------------------
...
if [[ -e $filename.unsorted ]]
then
	$(rm -f $filename.unsorted)
	if (( $? != 0 ))
	then
		echo "Error: Could not remove ${filename}.unsorted."
		exit 3
	else
		echo "Note: Removing existing file ${filename}.unsorted."
	fi
fi

if [[ -e $filename.sorted ]]
then
	$(rm -f $filename.sorted)
	if (( $? != 0 ))
	then
		echo "Error: Could not remove ${filename}.sorted."
		exit 3
	else
		echo "Note: Removing existing file ${filename}.sorted."
	fi
fi
...
read -r firstline1<$filename

arr4=$(echo $firstline1)
names=( $(echo ${arr4[@]:4}) )

while read -r line 
do
	temparr1=($(echo $line))
	temparr2=( $(echo ${temparr1[@]:1}) )
	x=0
	for item in ${names[*]}
	do
		echo "$item,${temparr1[0]},${temparr2[$x]}" >> ${filename}.unsorted
		((x=$x+1))

	done
done < <(tail -n +2 $filename)

echo "New Line" | sort -t"," -k3,3nr -k1 ${filename}.unsorted >> ${filename}.sorted
